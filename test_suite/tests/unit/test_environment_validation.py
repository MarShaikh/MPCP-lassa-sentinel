...
from src.batch_processing.batch_task_runner import get_work_items_from_file, main
from src.batch_processing.processing import upload_blob_to_azure, process_batch_with_progress
...
class TestRequiredEnvironmentVariables:
    ...
    def test_missing_storage_account_url_in_upload(self):
        """Test that upload_blob_to_azure fails when STORAGE_ACCOUNT_URL is missing."""
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            try:
                # Should raise KeyError when trying to access STORAGE_ACCOUNT_URL
                ...
                assert "STORAGE_ACCOUNT_URL" in str(exc_info.value)
            finally:
                os.unlink(temp_file)
    
    def test_missing_cog_container_sas(self):
        """Test that upload fails when COG_CONTAINER_SAS is missing."""
        env_vars = {
    ...
    def test_missing_raw_container_sas(self):
        """Test that upload fails when RAW_CONTAINER_SAS is missing."""
        env_vars = {
            'STORAGE_ACCOUNT_URL': 'https://test.blob.core.windows.net',
    ...
    def test_missing_logs_container_sas(self):
        """Test that upload fails when LOGS_CONTAINER_SAS is missing."""
        env_vars = {
            'STORAGE_ACCOUNT_URL': 'https://test.blob.core.windows.net',
            'COG_CONTAINER_SAS': 'test_sas',
            'RAW_CONTAINER_SAS': 'test_sas',
...
class TestInvalidEnvironmentVariables:
    ...
    def test_empty_sas_token(self, mock_blob_service):
        ...
        env_vars = {
            'STORAGE_ACCOUNT_URL': 'https://test.blob.core.windows.net',
            'COG_CONTAINER_SAS': '',  # Empty SAS token
        }
